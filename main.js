...
function graduation(sheet, key) {
    var dat = sheet.getDataRange().getValues(); //受け取ったシートのデータを二次元配列に取得
    var result = [];
    console.log((Number(key) + 1) * 10000);
    for (var i = 0; i < dat.length; i++) {
...
function doGet(e) {
    ...
    var template = (() => {
        try {
            if (user_permission == 'Privilege' || user_permission == 'Admin' || user_permission == 'Advisor') {
...
function getData() {
    ...
    switch (arguments[0]) {
        ...
        case 'schedule_list':
            findNearDataRow(schedule_db);
            schedule_db.getDataRange().sort({ column: 10, ascending: true });
            return [member_db.getRange(findRow(member_db, LOGIN_USER, 5), 7).getValue(), schedule_db.getRange(1, 1, schedule_db.getLastRow(), schedule_db.getLastColumn()).getValues()];
